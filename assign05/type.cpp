...
bool Type::is_int_variations() const
{
  return false;
}
...
bool QualifiedType::is_int_variations() const
{
  return get_base_type()->is_int_variations();
}
...
bool BasicType::is_int_variations() const
{
  return m_kind != BasicTypeKind::VOID;
}
...
unsigned StructType::get_field_offset(const std::string &name) const
{
  ...
  RuntimeError::raise("Attempt to get offset of nonexistent field");
}
