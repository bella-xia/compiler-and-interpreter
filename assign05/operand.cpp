...
Operand::Operand(Kind kind)
    : m_kind(kind), m_basereg(-1), m_index_reg(-1), m_imm_ival(-1), m_comment("")
{
}
...
bool Operand::operator<(const Operand &rhs) const
{
  ...
  case Operand::MREG64_MEM_OFF:
    return lhs.get_base_reg() < rhs.get_base_reg() ||
           (lhs.get_base_reg() == rhs.get_base_reg() && lhs.get_offset() < rhs.get_offset());

  case Operand::VREG_MEM_IDX:
  case Operand::MREG64_MEM_IDX:
    return lhs.get_base_reg() < rhs.get_base_reg() ||
           (lhs.get_base_reg() == rhs.get_base_reg() &&
            lhs.get_index_reg() < rhs.get_index_reg());
  ...
  default:
    std::cout << lhs.get_kind() << " " << rhs.get_kind() << std::endl;
    assert(false);
    return false;
  }
}
...
bool Operand::is_in_stack() const
{
  return (m_kind <= VREG_MEM_OFF) || (m_kind >= MREG64_MEM && m_kind <= MREG64_MEM_IDX_SCALE);
}
