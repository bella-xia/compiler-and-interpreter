...
void SemanticAnalysis::visit_variable_declaration(Node *n)
{
  ...
  for (auto i = declare_list->cbegin(); i != declare_list->cend(); ++i)
  {
    ...
    defined_symbol = m_cur_symtab->define(SymbolKind::VARIABLE, var_name, var_type);
    (*i)->set_symbol(defined_symbol);
  }
}
...
void SemanticAnalysis::visit_function_definition(Node *n)
{
  ...
  Node *func_name_node = n->get_kid(1);
  const std::string func_name = func_name_node->get_str();
  ...
  // func_name_node->set_symbol(func_symbol);
  ...
  func_members->set_symbol(func_symbol);
  ...
  for (auto i = func_members->cbegin(); i != func_members->cend(); i++)
  {
    ...
    (*i)->set_symbol(m_cur_symtab->define(param_kind, (*i)->get_str(), param_type));
  }
...
void SemanticAnalysis::visit_return_expression_statement(Node *n)
{
  ...
  {
    return_node->set_function_type(std::shared_ptr<Type>(expected_return_type));
...
void SemanticAnalysis::visit_unary_expression(Node *n)
{
  ...
  case TOK_AMPERSAND:
    ...
    if (var->get_tag() == AST_VARIABLE_REF)
    {
      assert(var->has_symbol());
      Symbol *ref_symbol = var->get_symbol();
      if (ref_symbol->is_vreg())
      {
        ref_symbol->set_to_stack();
      }
    }
    break;
  case TOK_MINUS:
...
void SemanticAnalysis::visit_function_call_expression(Node *n)
{
  ...
  for (auto i = arg_list->cbegin(); i != arg_list->cend(); ++i)
  {
    ...
    param = (param->is_array()) ? std::shared_ptr<Type>(new PointerType(param->get_base_type())) : param;
    std::shared_ptr<Type> dest_type(apply_assignment_rule(n, param, arg));
    (*i)->set_function_type(std::shared_ptr<Type>(dest_type));
    idx++;
  }
...
void SemanticAnalysis::visit_literal_value(Node *n)
{
  ...
  if (literal_val->get_tag() == TOK_INT_LIT)
  {
    ...
    n->set_literal_value(int_lit_val);
  }
  else if (literal_val->get_tag() == TOK_CHAR_LIT)
  {
    LiteralValue char_lit_val = LiteralValue::from_char_literal(literal_val->get_str(), literal_val->get_loc());
    std::shared_ptr<Type> lit_val_type(std::shared_ptr<Type>(new BasicType(BasicTypeKind::CHAR, true)));
    n->set_type(lit_val_type);
    n->set_literal_value(char_lit_val);
  }
  else
  {
    // TOK_STRING_LIT
    LiteralValue str_lit_val = LiteralValue::from_str_literal(literal_val->get_str(), literal_val->get_loc());
    std::shared_ptr<Type> lit_val_type(std::shared_ptr<Type>(
        new PointerType(std::shared_ptr<Type>(
            new QualifiedType(std::shared_ptr<Type>(
                              ...
                              TypeQualifier::CONST)))));
    n->set_type(lit_val_type);
    n->set_literal_value(str_lit_val);
    m_string_literal_map->insert(std::make_pair(literal_val->get_str().substr(1, (int)literal_val->get_str().size() - 2), n));
  }
  // TODO: implement
}
...
std::shared_ptr<Type> SemanticAnalysis::apply_assignment_rule(Node *n, std::shared_ptr<Type> lsymbol_type, std::shared_ptr<Type> rsymbol_type)
{
  ...
  else if (lsymbol_type->is_int_variations())
  {
    ...
    else if ((rsymbol_type->is_const() || rsymbol_type->is_volatile()) && rsymbol_type->get_base_type()->is_int_variations())
    {
      compatible = true;
    }
    else
  ...
  else if (lsymbol_type->is_pointer())
  {
    ...
    // }
    if (rsymbol_type->is_pointer() || rsymbol_type->is_array())
    {
      std::shared_ptr<Type> lsymbol_base_type(lsymbol_type);
      std::shared_ptr<Type> rsymbol_base_type(rsymbol_type);
      while (lsymbol_base_type->is_pointer() && (rsymbol_base_type->is_pointer() || rsymbol_base_type->is_array()))
      {
